...
public class BlockValiantSwitchGenerator extends NetworkDeviceGenerator {
    ...
    public BlockValiantSwitchGenerator(IntermediaryGenerator intermediaryGenerator, int numNodes) {
        SimulationLogger.logInfo("Network device", "BLOCK_VALIANT_SWITCH(numNodes=" + numNodes + ")");
    ...
    public NetworkDevice generate(int identifier, TransportLayer transportLayer) {
        ...
        return new BlockValiantEcmpSwitch(identifier, transportLayer, intermediaryGenerator.generate(identifier), blockID, this.numBlocks);
    }
