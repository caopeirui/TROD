...
public class BlockUgalLQueueBasedRouting extends BlockValiantRouting {

    //private HashMap<Integer, HashMap<Integer, List<Pair<Integer, Double>>>> routingWeights;

    public BlockUgalLQueueBasedRouting(Map<Integer, NetworkDevice> idToNetworkDevice) {
        // initialize the parent class
        super(idToNetworkDevice);
        SimulationLogger.logInfo("Routing", "BLOCK_UGALL_QUEUE_BASED_ROUTING");
    }
    ...
    public void populateRoutingTables() {
        ...
        for (int identifier : idToNetworkDevice.keySet()) {
            BlockUgalLQueueBasedSwitch device = (BlockUgalLQueueBasedSwitch) idToNetworkDevice.get(identifier);
            if (!device.isServer()) {
        ...
        for (int serverID : details.getServerNodeIds()) {
            BlockUgalLQueueBasedSwitch device = (BlockUgalLQueueBasedSwitch) this.idToNetworkDevice.get(serverID);
        ...
        for (int identifier : this.idToNetworkDevice.keySet()) {
            BlockUgalLQueueBasedSwitch device = (BlockUgalLQueueBasedSwitch) this.idToNetworkDevice.get(identifier);
        ...
        // only valid for BlockUgalLQueueBasedSwitch.
        for (int blockID : blockIDToSwitchIDs.keySet()) {
            ...
            for (int switchID : switchIDsInBlock) {
                BlockUgalLQueueBasedSwitch currentSwitchEntity = (BlockUgalLQueueBasedSwitch) idToNetworkDevice.get(switchID);
