...
public class ReconfigurablePodInfinibandRoutingUtility {

    private static final int INFINITY = 999999999;

    public ReconfigurablePodInfinibandRoutingUtility() {
    }
                ...
                if (i != j) {
                    ...
                    for (Vertex v : adjacent) {
                        ...
                        if (dist_ij == distanceMatrix.get(v.getId()).get(j) + 1) {
                            ((ReconfigurableInfinibandSwitch) idToNetworkDevice.get(i)).addDestinationToNextSwitch(j, v.getId());
                        }
                    }
                }
            }
     ...
     */
    public static void populateIntraPodRoutingTables(Map<Integer, NetworkDevice> idToNetworkDevice, 
